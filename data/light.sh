...
Main () {
    ...
    SaveGamma="1.0:1.0:1.0"

    Brightness=$MinBright
    Gamma=$(echo $SaveGamma | cut -d ':' -f 1)

    # Immediately dim to black.
    xrandr --output "$MonXrandrName" --gamma "$Gamma:$Gamma:$Gamma" \
    --brightness "$MinBright"

    sleep .1

    # Slowly get bright...
    while [[ $(echo "$Brightness < $MaxBright" | bc) -eq 1 ]] ; do
        Brightness=$(echo "$Brightness + 0.1" | bc)
        Gamma=$(echo "$Gamma + 0.01" | bc)
    ...
    while [[ $(echo "$Brightness > $SaveBrightness" | bc) -eq 1 ]] ; do
        Brightness=$(echo "$Brightness - 0.1" | bc)
        Gamma=$(echo "$Gamma - 0.01" | bc)
